...
import { checkout, consult_order, order_success, product } from '../../interface/models';
  ...
  getOrder(sessionId:string){
    return this.getToken().pipe(
      switchMap((res: any) => {
        ...
        const url = `${this.baseUrl}/api/get-orders?session_id=${sessionId}`;
        return this.http.get<order_success[]>(url, { headers });
      })
    );
  }

  consultOrder(code_order:string){
    return this.getToken().pipe(
      switchMap((res: any) => {
        const token = res.message;
        if (!token) throw new Error("Token no recibido");
        ...
        const url = `${this.baseUrl}/api/consult-order?code_order=${code_order}`;
        return this.http.get<consult_order>(url, { headers });
      })
    );
  }
}
